...
# commit by kevin 
# 和公网版本的 阿里云 DDNS 的脚本差不多，就将获取 ipv4 公网 ip 变成了获取内网的 ip 
# 会创建一个 local_ip 文件，所以得先确保有写入的权限，麻烦了点，如果有更好的方法请联系 kevin_szu@163.com
...
function_AliDDNS_SetConfig(){
    ...
    if [ "${Switch_AliDDNS_ExpertMode}" = "1" ]; then
        ...
        ifconfig | grep inet | grep -oE '172\.[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}' | head -n1 > local_ip
        AliDDNS_LocalIP="cat local_ip"
    fi
    # AliDDNS_DomainServerIP
    if [ "${Switch_AliDDNS_ExpertMode}" = "1" ]; then
